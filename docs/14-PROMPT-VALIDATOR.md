...
0. **Konfirmasi Pemahaman**: Sebagai langkah paling awal, AI wajib:
   - Menyatakan pemahamannya secara jelas tentang prompt yang diterima.
   - Menyebutkan fitur apa yang akan dikerjakan.
   - Menyebutkan file atau section dokumentasi mana yang akan dijadikan referensi.
   - Konfirmasi ini disajikan secara *inline* di bagian atas Laporan Validator dalam satu balasan yang sama, sehingga tidak perlu menunggu persetujuan terpisah.
1. **Analisis Prompt**: Mengidentifikasi modul/fitur apa saja yang akan terpengaruh oleh permintaan user.
2. **Cross-check Dokumentasi**: AI **hanya** membaca file `docs/*.md` yang relevan dengan prompt saja (bukan semua file sekaligus).
   - AI wajib menyebutkan file mana yang dibaca beserta alasan kenapa file tersebut relevan.
   - Jika tidak ada file yang relevan, AI harus menyatakannya secara eksplisit.
